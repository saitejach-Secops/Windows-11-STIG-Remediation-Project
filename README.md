...
All PowerShell remediation scripts are in [`scripts/`](scripts/). Each script targets a grouped set of related STIGs and includes output confirmation.
...
Each STIG was individually verified with before/after evidence screenshots available in the [`evidence/`](evidence/) folder and linked from each group's documentation page.
...
| Scan finishes in 1 min with no results | Azure blocks ICMP by default — uncheck "Ping the remote host" in Tenable Host Discovery settings |
...
├── docs/
│   ├── Group1-EventLogSizes.md
│   ├── Group2-PowerShellLogging.md
│   ├── Group3-CredentialSecurity.md
│   ├── Group4-ProcessAuditing.md
│   └── Group5-InstallerPrivileges.md
└── evidence/
    ├── baseline-scan.png
    ├── group1/  (12 screenshots)
    ├── group2/  (9 screenshots)
    ├── group3/  (7 screenshots)
    ├── group4/  (4 screenshots)
    └── group5/  (3 screenshots)
```
